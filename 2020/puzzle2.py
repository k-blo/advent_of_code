...
valid = 0
valid_according_second_policy = 0
...
for pw in passwords:
    ...
    letter = policy[1]
    numbers = policy[0]
    min_max_list = numbers.split("-")
    min = int(min_max_list[0])
    max = int(min_max_list[1])

    counted = password.count(letter)

    if counted >= min and counted <= max:
        valid += 1
    

    password = password.split()[0] # remove empty space and convert to string
    ...
    if a is not b:
        valid_according_second_policy += 1
...
print (valid_according_second_policy)
